...
def dump(data: Dump):
    ...
    for i, v in enumerate(data.voices):
        ...
        # Используем shell=True для Windows или проверяем наличие ffmpeg
        try:
            subprocess.run(
                ["ffmpeg", "-y", "-i", ogg, mp3], 
                check=True,
            ...
            # Если ffmpeg не найден в PATH, пробуем с полным путем
            subprocess.run(
                ["ffmpeg.exe" if os.name == 'nt' else "ffmpeg", "-y", "-i", ogg, mp3], 
                check=True,
                capture_output=True,
                shell=True
            )
